...
opt_minfbe_bfgs = baseopt; opt_minfbe_bfgs.solver = 'minfbe'; opt_minfbe_bfgs.method = 'bfgs';
out_minfbe_bfgs = forbes(f, g, x0, aff, [], opt_minfbe_bfgs);

assert(norm(out_fbs.x - out_minfbe_bfgs.x) <= ASSERT_TOL);

opt_minfbe_lbfgs = baseopt; opt_minfbe_lbfgs.solver = 'minfbe'; opt_minfbe_lbfgs.method = 'lbfgs';
out_minfbe_lbfgs = forbes(f, g, x0, aff, [], opt_minfbe_lbfgs);

assert(norm(out_fbs.x - out_minfbe_lbfgs.x) <= ASSERT_TOL);
