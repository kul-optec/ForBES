...
%     inital_out = forbes_qp(P, q, A, lb, ub,[],[], [], [], opt1);
    ...
    some = new{k};
    results(k,2) = out_maha{k}.solver.solver.gam;
    results(k,3) = out_maha{k}.solver.solver.time;
    results(k,4) = out_osqp{k}.info.run_time;
    results(k,5) = out_maha{k}.solver.solver.iterations;
    results(k,6) = out_osqp{k}.info.iter;
    results(k,7) = out_maha{k}.solver.solver.residual(end);
    results(k,8) = out_osqp{k}.info.pri_res;
    results(k,9) = -out_maha{k}.solver.solver.objective(end);
    results(k,10) = out_osqp{k}.info.obj_val;
    results(k,11) = out_maha{k}.status;
    results(k,12) = out_osqp{k}.info.status_val;
    results(k,13) = err_nama;
    results(k,14) = err_osqp;
